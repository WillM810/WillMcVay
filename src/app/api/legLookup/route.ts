...
const DIS_URL =
    `https://enterprise.firstmap.delaware.gov/arcgis/rest/services/Boundaries/DE_Political_Boundaries/MapServer/XX/` +
    `query?f=json&geometryType=esriGeometryPoint&outFields=*&geometry=`;

export async function POST(request: Request) {
    const { address, location, spatialReferenceWkID } =
        await lookupAddress(request);
...
async function lookupAddress(request: Request) {
    ...
    const geoResponse = await fetch(
        `${GEO_URL}${encodeURIComponent(addressQuery)}`,
    );
    ...
    const layerData = await Promise.all(
        [0, 1, 2, 3].map(async (layer) => {
            const response = await fetch(
                `${DIS_URL.replace("XX", layer.toString())}${location.x},${location.y}&inSR=${wkID}`,
            );
